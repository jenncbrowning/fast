...
ui <- fluidPage(
  ...
  sidebarPanel(
    ...
    br(),
    helpText(h4("Model:")),
    div(textOutput("model"), style = "text-indent:40px;font-size:125%;"),
    br(),
    uiOutput("tail"),
...
server <- function(input, output) {
  ...
  output$lower_bound <- renderUI({
    ...
    if (input$dist == "rbinom") {
      ...
      if (input$tail %in% c("both", "middle")) {
        radioButtons(
          inputId = "lower_bound",
          label = h5("Lower bound:"),
          choiceNames = list(
            "X < a", "X \u2264 a"),
          choiceValues = list("open","closed"),
          selected = "open")
      }
  ...
  output$sd <- renderUI({
    # print("sd")
    if (input$dist == "rnorm") {
      numericInput("sd",label=h5('Standard Deviation'),
        value = 1,min = 0.1,max = 500,step = 0.1)
    }
  })
